...
namespace Clinic.Bot.Dialogs
{
    public class RootDialog : ComponentDialog
    {
        ...
        public RootDialog(ILuisService luisService, IDataBaseService dataBaseService, UserState userState, IQnAMakerAIService qnAMakerAIService)
        {
            ...
            {   // Contiene todos los métodos que se van a ejecutar secuencialmente
                InitialProcess,
                FinalProcess
            };
            // Si se usa un diálogo que no está declarado acá, saldrá un error
            AddDialog(new WaterfallDialog(nameof(WaterfallDialog), waterfallSteps));
            AddDialog(new QualificationDialog(_dataBaseService));
            AddDialog(new CreateAppointmentDialog(_dataBaseService, _userState, _luisService));
            AddDialog(new TextPrompt(nameof(TextPrompt)));
            InitialDialogId = nameof(WaterfallDialog); // Indicar que debe inicializar con el WaterfalDialog
        }
        ...
        private async Task<DialogTurnResult> ManageIntentions(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            ...
            switch (topIntent.intent)
            {
                ...
                case "VerCentroContacto":
                    ...
                    break;
                case "Calificar": //acá ponemos return porque vamos a trabajar con diálogos externos y estos devuelven una actividad
            ...
            return await stepContext.NextAsync(null, cancellationToken); // Para que salte al siguiente método
        }
        ...
        private async Task IntentVerCita(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            ...
            {
                var pending = medicalData.Where(x => x.date >= DateTime.UtcNow.AddHours(-5).Date).ToList(); // && x.time > DateTime.UtcNow.AddHours(-5).Hour
        ...
        private async Task IntentVerCentroContacto(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            string phoneDetail = $"Nuestros número de atención son los siguientes:{Environment.NewLine}" +
                $"📞 +51 987654321{Environment.NewLine}📞 +51 987654321";

            string addressDetail = $"🏥 Estamos ubicados en:{Environment.NewLine} Calle Chura 345, Lima";
