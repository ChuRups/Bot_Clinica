...
namespace Clinic.Bot.Dialogs.CreateAppointment
{
    public class CreateAppointmentDialog : ComponentDialog
    {
        ...
        public CreateAppointmentDialog(IDataBaseService dataBaseService, UserState userState, ILuisService luisService)
        {
        ...
        private async Task<DialogTurnResult> SetPhone(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                string text = $"Por favor ingresa tu número de teléfono en el siguiente formato:" +
                 ...
                 $"{Environment.NewLine}Ejemplo: +51 987654321";
        ...
        private async Task<DialogTurnResult> SetFullName(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Ahora ingresa tu nombre completo:") }, cancellationToken);
            }
        }
        ...
        private async Task<DialogTurnResult> SetEmail(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Ahora ingresa tu correo:") }, cancellationToken);
            }
        }
        ...
        private async Task<DialogTurnResult> SetDate(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (Entities.datetime != null) // Acá es cuando reconoce la fecha en la entidad 
            {
                var date = Entities.datetime.First().timex.First().Replace("XXXX", DateTime.UtcNow.AddHours(-5).Year.ToString());
                medicalAppointment.date = DateTime.Parse(date);
                return await stepContext.NextAsync(cancellationToken: cancellationToken);
            }
            else // Si la fecha no se dijo por entidad
            {
        ...
        {
            if (medicalAppointment.date == DateTime.MinValue) //Si ya se capturó la fecha, no debemos sobreescribirla
            {
                var medicalDate = stepContext.Context.Activity.Text;
                medicalAppointment.date = DateTime.ParseExact(medicalDate, "dd/MM/yyyy", null); //Convert.ToDateTime(medicalDate); // Sale error en Azure por el formato diferente MM/dd/yyyy
            }

            return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = CreateButtonsTime() }, cancellationToken);
        }
        ...
        private async Task<DialogTurnResult> Confirmation(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = CreateButtonsConfirmation() }, cancellationToken);
        }
        ...
        private async Task<DialogTurnResult> FinalProcess(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (userConfirmation.ToLower().Equals("si"))
                {
                    ...
                    //await SendEmail(userModel, medicalAppointment); //ENVIAR CORREO
            ...
            return await stepContext.ContinueDialogAsync(cancellationToken: cancellationToken); // Para que regrese al dialogo root
        }
